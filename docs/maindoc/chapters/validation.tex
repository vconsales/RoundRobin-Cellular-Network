...
This is a test to ultimately validate the model shown in the previous chapter. Build a full matematical model starting from the requirements it's too difficult, but having no one is not helpful in explaining the simulation results. So we started to build simple models leaving off some of the requirements and targeting only some conditions (like saturation). In our case one of the most difficult things to model is packet framing: as the requirements said, every RB cannot contain packets from two or more different users. This means that there will be some wasted space to consider. Furthermore, if a packet cannot completely fill the frame, it cannot be scheduled and the scheduler moves to the next user.
...
As we said, \(packetsize=1\) is a solution, however another interesting solution is \(packetsize=25\). This is due to the fact the \(\#RB\) is fixed and used to compute every client frame size. Note that this is valid only if frame is filled every time by a single client (as the previous assumption). Lets check the results:
...
\[E[th_{antenna}] = \frac{\sum_{i=1}^{\#clients} 25*E[rbsize_i]}{\#clients*(1/T_{slot})} \]
\(rbsize_i\) is a RV variable with an not known note distribution, however
\[\forall i, \quad rbsize_i = f(CQI \sim U(1,15))\]
...
\[\forall i, \quad E[rbsize_i] = 40.6\ byte/s\]
which is constant for all clients. In this case the initial model can be simplified to
\[E[th_{antenna}] = \frac{25*E[rbsize_i]}{1/T_{slot}} *8 = 8.12\ Mbit/s\]
